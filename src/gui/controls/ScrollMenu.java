...
class ScrollMenu extends Control implements ControlListener, MouseMotionListener
{
    ...
    public void paint (Graphics g)
    {
        ...
        g.setColor (ColorUtil.scale (this.color, 0.10));
        g.fillRect (this.leftX + 1, this.topY + 1, this.width - 1 - SCROLL_BAR_WIDTH, this.height - 1);
        
        // Scroll Bar Background
        g.setColor (ColorUtil.scale (this.color, 0.20));
        ...
        // Scroll Bar
        g.setColor ((this.enableScrollDragging) ? ColorUtil.scale (this.color, 0.50) : ColorUtil.scale (this.color, 0.40));
        g.fillRect (this.rightX - SCROLL_BAR_WIDTH, scrollY, SCROLL_BAR_WIDTH, this.scrollBarHeight);

        // Borders
        g.setColor (ColorUtil.scale (this.color, 0.75));
    ...
    public void mousePressed (MouseEvent e)
    {
        ...
        if (!this.scrollDisabled && this.overScroll (x, y))
        {
            this.enableScrollDragging = true;
        }
        else if (!this.scrollDisabled && this.overScrollBar (x, y))
        {
